...
class Operadores:
    ...
    def leer_conjunto(self, nombre):
          valores = input(
              f"Ingrese los datos del conjunto {nombre} separados por comas ")
...
class LectorConjuntos:
    def leer_conjunto(self, valores_str):
        try:
            ...
            numeros = [int(x.strip())
                       for x in valores_str.split(",") if x.strip()]
            return set(numeros)
        except ValueError:
...
class InterfazConjuntos(tk.Tk):
    ...
    def crear_componentes(self):
        ...
        tk.Label(frame_a, text="Conjunto A:",
                 font=("Arial", 12, "bold")).pack()
        tk.Label(
            frame_a, text="Ingresa números separados por comas (ej: 1,9,20,53,411)").pack()
        self.entry_a = tk.Entry(frame_a, width=50, font=("Arial", 10))
        self.entry_a.pack(pady=5)

        frame_b = tk.Frame(self)
        frame_b.pack(pady=10)
        tk.Label(frame_b, text="Conjunto B:",
                 font=("Arial", 12, "bold")).pack()
        tk.Label(
            frame_b, text="Ingresa números separados por comas (ej: 2,8,19,50,400)").pack()
    ...
    def mostrar_resultado(self, titulo, resultado):
        ...
        if isinstance(resultado, bool):
            self.texto_resultados.insert(
                tk.END, f"Respuesta: {'SÍ' if resultado else 'NO'}")
